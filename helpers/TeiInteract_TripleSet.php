...
class TeiInteract_TripleSet {
    ...
    public static function getTriplesSetForTitle($title) {
        ...
        if(count($tripleSet->rootItems)>0){
            debug(sprintf(
                "%s - found %d matches for search term '%s'; first found match ID = %d", 
                get_class(), count($tripleSet->rootItems), $title, $tripleSet->rootItems[0]->id));

            $tripleSet->triples = array();
            debug(sprintf(
                "%s - processing %d item records into one triple Set", 
                get_class(), count($tripleSet->rootItems)));
            
            foreach($tripleSet->rootItems as $item) {  
                debug(sprintf(
                    "beginning to build triples sourced from item with id = %d",
                    $item->id));

                $tripleSet->triples[$item->id] = array();
                
                $irirTbl    = new ItemRelationsItemRelationTable(
                    'ItemRelationsItemRelation', get_db());
